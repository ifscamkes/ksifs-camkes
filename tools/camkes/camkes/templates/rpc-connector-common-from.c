...
/*- endif -*/
    ...
    //TODO: (#RWFM) check for read and write rule accordingly here and 
    //remove the checking from internal code.
    ...
    int status = rwfm_CheckReadRule(intNo, compNo);
    if (status == 1/*FAILURE*/) {
      //printf("RWFM Read Rule Failure\n");
      /*- if m.return_type == 'string' -*/
	return 0;
      /*- else -*/
	return ;
    ...
    status = rwfm_CheckWriteRule(compNo, intNo);
    if (status == 1/*FAILURE*/) {
    //printf("RWFM Write Rule Failure\n");
     /*- if m.return_type == 'string' -*/
	return 0;
     /*- else -*/
	return ;
    ...
    rwfm_PerformLabelUpdate(intNo, compNo);
